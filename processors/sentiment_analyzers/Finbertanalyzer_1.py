...
class FinBERTProsusAnalyzer(BaseAnalyzer):
    def __init__(self, classifier):
        # Build the pipeline here; no truncation — we chunk to <=512 tokens ourselves
        self.classifier = classifier
        self.tokenizer = classifier.tokenizer

    def split_into_chunks(self, text: str, max_tokens: int = 480, stride_tokens: int = 64) -> list[str]:
        """
        Sentence-aware token chunking with small overlap (stride).
        max_tokens < 512 to leave margin for special tokens.
        """
        ...
        for sent in sentences:
            ...
            # If a single sentence is too long, hard-split it by tokens
            if s_len > max_tokens:
                if current:
            ...
            if cur_len + s_len <= max_tokens:
                ...
                if current:
                    chunks.append(" ".join(current))
                # start new with overlap (reuse last short sentence as context if possible)
    ...
    def _hard_token_split(self, text: str, max_tokens: int, stride_tokens: int) -> list[str]:
        ...
        while start < len(ids):
            ...
            if end == len(ids):
                break
            # CORRECTED LOGIC: Always slide the window forward
            start = end - stride_tokens
        return out

    def analyze(self, text_sections: list[dict]) -> dict:
        """
        Returns speaker-level results by accumulating all chunks for a speaker
        ...
        for section in text_sections:
            ...
            chunks = self.split_into_chunks(speech)
