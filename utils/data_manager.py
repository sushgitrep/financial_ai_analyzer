...
class DataManager:
    ...
    def _get_storage_stats(self, bank_key: str) -> Dict[str, Any]:
        try:
            ...
            data_files = (
                len(list(data_path.glob("*.json"))) if data_path.exists() else 0
            )
    ...
    def save_analysis_results(
        self, bank_key: str, data_type: str, results: Dict[str, Any]
    ) -> bool:
        try:
            data_path = self.base_path / "data" / "banks" / bank_key
    ...
    def load_analysis_results(
        self, bank_key: str, data_type: str
    ...
    def save_plot(
        self, bank_key: str, plot_name: str, fig, plot_type: str = "matplotlib"
    ) -> bool:
        ...
        try:
            ...
            if plot_type == "plotly":
                filename = f"{plot_name}_{timestamp}.json"
                filepath = plot_path / filename
    ...
    def get_available_plots(
        self, bank_key: str, extension: str = "png"
    ) -> Dict[str, List[str]]:
        try:
            plot_path = self.base_path / "plots" / bank_key
            if plot_path.exists():
